...
while($row = mysql_fetch_assoc($result)){
  $q1 = sprintf ("BEGIN");
  mysql_query($q1);
  $q2 = sprintf ("DELETE FROM incident");
  mysql_query($q2);
  ...
  while($cat = mysql_fetch_assoc($rcat)){
    $q4 = sprintf ("INSERT INTO incident SELECT id,location_id,form_id,locale,user_id,%s,incident_title,incident_description,incident_date,incident_mode,incident_active,incident_verified,incident_source,incident_information,incident_rating,incident_dateadd,incident_dateadd_gmt,incident_datemodify FROM %s.incident WHERE id IN (SELECT incident_id FROM %s.incident AS A JOIN %s.incident_category ON incident_id = A.id WHERE category_id = %s AND incident_active)"
  ...
  $q7 = sprintf ("REPLACE INTO incident_person SELECT * FROM %s.incident_person",
      mysql_real_escape_string($row['dbdatabase']));
  mysql_query($q7);
  $q8 = sprintf ("REPLACE INTO location SELECT * FROM %s.location",
       mysql_real_escape_string($row['dbdatabase']));
  mysql_query($q8);
  $q9 = "COMMIT";
  mysql_query($q9);
